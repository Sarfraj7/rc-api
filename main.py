...
async def fetch_vehicle_data(reg_no: str, vertical: str, client: httpx.AsyncClient):
    ...
    try:
        ...
        if "application/json" not in response.headers.get("content-type", ""):
            return {"internal_api_error": f"Server returned non-JSON. Status: {response.status_code}. Data might be blocked or Cookie expired."}
        return response.json()
    except Exception as e:
        return {"internal_api_error": f"Failed to connect: {str(e)}"}
...
async def get_rc_details(reg_no: str = Query(..., description="Vehicle Registration Number")):
    ...
    async with httpx.AsyncClient(verify=False) as client:
        ...
        # Check if we caught an error in the safety net
